...
def compute_edge_features(
    ...
    """
    Compute edge features from positions and topology.

    Args:
        pos: (N, 3) node positions
        edge_index: (2, E) edge indices
        cutoff: Distance cutoff for RBF encoding
        num_rbf: Number of RBF basis functions

    Returns:
        edge_dist: (E,) distances
        edge_unit: (E, 3) unit vectors
        edge_rbf: (E, num_rbf) RBF features
    """
    ...
    if edge_index.size(1) == 0:
        return (
            ...
            torch.empty((0, num_rbf), dtype=torch.float32),
        )

    src, dst = edge_index[0], edge_index[1]
    vec = pos[dst] - pos[src]
    dist = torch.linalg.norm(vec, dim=-1).clamp(min=1e-4)
    unit = vec / dist[:, None]
    rbf_feat = rbf(dist, num_gaussians=num_rbf, cutoff=cutoff)

    return dist, unit, rbf_feat
...
def _pad_atom_embeddings_for_mates(
    ...
    pad = asu_embedding.new_zeros(total_num_atoms - asu_embedding.size(0), asu_embedding.size(1))
    ...
    try:
        ...
        for i in range(len(water_atoms)):
            ...
            if key not in bfactor_lookup:
                ...
                normalized = (raw_bfactor - water_mean) / water_std if water_std > 0 else 0.0
...
class ProteinWaterDataset(Dataset):
    ...
    def _preprocess_one(self, entry: dict, cache_path: Path):
        ...
        any_filter_enabled = use_distance_filter or use_edia_filter or use_bfactor_filter
        ...
        if any_filter_enabled and water_atoms:
            ...
            water_keys = list(zip(
                ...
                np.array([_normalize_ins_code(x) for x in water_atoms.ins_code], dtype=object),
            ))
        ...
        ins_code_arr = np.array([_normalize_ins_code(x) for x in protein_atoms.ins_code], dtype=object)
        ...
        if self.include_mates and mate_pos.size(0) > 0:
            ...
            final_protein_res_idx = torch.cat([protein_res_idx, offset_mate_res_idx], dim=0)
        else:
        ...
        if final_protein_pos.size(0) > 0:
            ...
            _, pp_edge_unit, pp_edge_rbf = compute_edge_features(
                final_protein_pos, pp_edge_index, cutoff=self.cutoff
            )
        else:
