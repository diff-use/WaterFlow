...
class Dim6RotStructureHead(nn.Module):
    ...
    def forward(self, x, affine, affine_mask, **kwargs):
        ...
        # Expect shape [B, L] here for valid/padded residues
        # Unsqueeze -> [B, L, 1] (can broadcast with [B, L, 12])
            if affine_mask.dim() == rigids.tensor.dim() - 1:
                affine_mask = affine_mask.unsqueeze(-1)
            elif affine_mask.dim() < rigids.tensor.dim() - 1:
                # e.g. if it's [B, L], just unsqueeze once
                affine_mask = affine_mask.unsqueeze(-1)
            # else: handle other edge cases as needed

            # DO NOT DO THIS!!!
            #affine_mask = affine_mask.expand_as(rigids.tensor)
            # e.g. now shape [B, L, 12] matches rigids.tensor [B, L, 12]
        """
        # squeeze affine_mask to [B, L]
        affine_mask = affine_mask.squeeze(-1)

        #logger.info(f"affine_mask shape: {affine_mask.shape}")
        #logger.info(f"rigids.tensor shape: {rigids.tensor.shape}")
        #logger.info(f"update.mask(affine_mask) shape: {update.mask(affine_mask).shape}")
        ...
        return affine, pred_xyz, angles    # Adjust mask dimensions to align with rigids.tensor
