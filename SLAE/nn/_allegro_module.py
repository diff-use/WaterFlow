...
from SLAE.nn.mlp_encoder import ScalarMLPFunction
from SLAE.nn.strided_irreps_ops import Contracter, MakeWeightedChannels, Linear
from SLAE.nn.cutoffs import cosine_cutoff, polynomial_cutoff_broadcasted as polynomial_cutoff
...
class Allegro_Module(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        field: str = "edge_attr", #AtomicDataDict.EDGE_ATTRS_KEY, # initial input is the edge SH
        edge_invariant_field: str = "edge_embedding", #AtomicDataDict.EDGE_EMBEDDING_KEY,
        node_invariant_field: str = "node_features", #AtomicDataDict.NODE_ATTRS_KEY,# TODO
        ...
        latent_out_field: Optional[str] = "edge_energy",#_keys.EDGE_FEATURES,
        ...
        self._init_irreps(
            ...
            required_irreps_in=[
                ...
                self.node_invariant_field, #node_features
            ],
        )

        # for normalization of env embed sums
        # one per layer
        """ # TODO moved to forward
        ...
        self.final_latent = latent(
            mlp_input_dimension=self.latents[-1].out_features + env_embed_multiplicity * n_scalar_outs,
            mlp_output_dimension=None, # TODO
        )
        # final_latent.out_features == mlp_latent_dimensions
        # - end build modules -
        ...
        self._latent_dim = self.final_latent.out_features   # TODO: check this, need larger than 1
    ...
    def forward(self, batch: Batch) -> Batch:
        ...
        env_sum_normalizations = torch.full(
            (self.num_layers,),
        ...
        edge_neighbor = batch.edge_index[1]

        edge_attr = getattr(batch, self.field)
        ...
        edge_length = batch.edge_distance #data[AtomicDataDict.EDGE_LENGTH_KEY] # TODO check this for consistency
        num_edges: int = len(edge_attr)
        #edge_invariants = batch.embedding #batch[self.edge_invariant_field] # TODO
        #node_invariants = batch.node_features # batch[self.node_invariant_field] #TODO
        edge_invariants = getattr(batch, self.edge_invariant_field)
        node_invariants = getattr(batch, self.node_invariant_field)
        ...
        for latent, env_embed_mlp, env_linear, tp, linear in zip(
            ...
            if env_sum_normalizations.ndim < 2:
                # it's a scalar per layer
                norm_const = env_sum_normalizations[layer_index]
            else:
                # it's per type
                # get shape [N_atom, 1] for broadcasting
                norm_const = env_sum_normalizations[
                    layer_index, batch.atom_types#data[AtomicDataDict.ATOM_TYPE_KEY] #TODO
                ].unsqueeze(-1)
        ...
        setattr(batch, self.latent_out_field, latents)
